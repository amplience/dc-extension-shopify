...
/**
 * Cors-enabled endpoint w/ 'soft' auth that fetches products by Ids
 * 
 * TODO: Needs to account for a possibly deleted product in Shopify.
 * 
 * NOTE:
 * Expects product aliases as EXACTLY: 'product{index}'
 * Expects queries to match Product type defined in models/Product.tsx
 * 
...
export default async function handler(req:NextApiRequest, res:NextApiResponse) {
    ...
    try {
        if(req.method === 'GET') {
            ...
            const products:any[] =  []
            // Extract `product${index}` aliases into Product[] type
            Object.keys(body.data).forEach(currentKey => {
                if(currentKey.includes("product")) {
                    products.push({
                        ...body.data[currentKey]
                    })
                }
            })
